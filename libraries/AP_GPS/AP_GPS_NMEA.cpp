...
bool AP_GPS_NMEA::_term_complete()
{
        ...
        if (checksum == _parity) {
            if (_gps_data_good) {
                ...
                case _GPS_SENTENCE_HDT:
                    _last_HDT_ms = now;
                    _ground_hdt_course = wrap_360(_new_hdt_course*0.01f);
                    break;
                }
            } else {
                switch (_sentence_type) {
    ...
    if (_sentence_type != _GPS_SENTENCE_OTHER && _term[0]) {
        ...
        case _GPS_SENTENCE_HDT + 1: // Course (HDT)
            _new_hdt_course = _parse_decimal_100(_term);
            break;
...
float AP_GPS_NMEA::get_hdt_course() const {
    return _ground_hdt_course;
}
